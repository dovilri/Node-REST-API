...
const GetAll = (req, res) => {
  PeriodicTable.find({}, 'atomicnumber name symbol weight -_id').sort( { id: 1 } )
    .then(result => {
      res.send(result);
    })
    .catch(() => {
...
const GetRandom = (req, res) => {
    ...
    PeriodicTable.findOne({ atomicnumber: RandomElementAtomicNumber }, 'atomicnumber name symbol weight -_id')
    .then(result => {
      if(result){
          res.send(result);
        }
